...
def selectFromDb():
    ...
    conection = sqlite3.connect("./basquet.db")
...
def send(name, surname, tall, years):
    ...
    try:
        ...
        cursor.execute("INSERT INTO jugadors (nom, cognom, alcada, edat) VALUES (?, ?, ?, ?)",
                       (name, surname, finalTall, finalYears))
        conection.commit()
        conection.close()
...
style.map('TButton', foreground=[('active', '!disabled', 'Navy Blue')],
          background=[('active', 'Navy Blue'), ('!disabled', 'black')], )
connectDB()
...
sendButton = ttk.Button(text="Enviar",
                        command=lambda: send(nameEntry.get(), surnameEntry.get(), tallEntry.get(), yOldEntry.get()))
